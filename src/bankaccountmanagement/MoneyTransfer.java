        ...
        try {
            ...
            if (AmountText.getText().length() == 0 || ConfirmText.getText().length() == 0 || PassText.getText().length() == 0) {
                ...
                if (JOptionPane.YES_OPTION == dialogResult) {
                    ...
                    if (input == null)//check for null first
                    {
                        //cancel, esc or X selected
                        //return; is a possibility
                    } else if (input.length() == 0)//if it is null, you do not want this to proceed, so it's 'else if'
                    {
                        //error message, and redisplay optionPane, is a possibility
                        ...
                        if (cstmt.getInt(2) == 1) {
                            ...
                            sql = "create or replace PROCEDURE TRANSFER(p_sender IN NUMBER, p_receiver IN NUMBER, p_amount IN NUMBER)\n"
                                    + "IS\n"
                                    + "BEGIN\n"
                            ...
                            sql = "SELECT BALANCE FROM ACCOUNT WHERE ACC_NO = '" + jLabel24.getText() + "'";
